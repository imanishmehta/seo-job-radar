...
Google discovery is OFF by default, even if a key is saved. To opt in, add a **Serper** API key as repository Actions secret `SERPER_API_KEY` and a repository variable `ENABLE_GOOGLE_DISCOVERY=true`, then run **Refresh career pages and deploy**. Set the variable to `false` to turn paid search off. The integration searches Google for SEO career pages and excludes known job portals. Search candidates appear under Career sources. Companies are automatically added when their homepage links to the career section and same-domain Organization metadata supplies a supported non-India company address. Up to 6 candidates are checked and 3 companies added per run. Unknown ownership/country, blocked sites and intermediaries remain in review. New sources are crawled in the same run; search results themselves never become jobs. Pending candidates are retried, and existing sources continue without search credits. Existing employer crawls need no search key.
